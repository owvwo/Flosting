...
function MatchingList(props) {
  ...
  const userChecker = async () => {
    const checkUserOne = await db
      .collection(`${Ongoing}${sort}`)
      .where("userOne.Unique_key", "==", uniqueKey)
      .get();
    try {
    ...
    const checkUserTwo = await db
      .collection(`${Ongoing}${sort}`)
      .where("userTwo.Unique_key", "==", uniqueKey)
      .get();
    try {
    ...
    if (user) {
      ...
      if (Ongoing && uniqueKey) {
        userChecker().then((_) => {
          ...
          매칭결과나머지배열변경(
            copy매칭결과배열.slice(Number(OriginalTicket))
          );
        });
      }
    }
  ...
  let matchingList = 매칭결과배열.map((list) => (
    <Fade bottom>
      <NavLink
        to={`/selectresult/${sort}/${Ongoing}/${list.DocId}`}
        style={{ textDecoration: "none" }}
      >
        <Parent>
          <div className="confirm">확인하기</div>
          <div className="Nick">
            <li>{list.Nick}</li>
          </div>
