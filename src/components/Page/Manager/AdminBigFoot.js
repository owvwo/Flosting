...
function AdminBigFoot(){
    ...
    const onChange = (event) => {
        ...
        if(name === '마감시간'){ 수정마감시간변경(value)}
        else if(name === '진행중회차'){ 수정진행중회차변경(value)}
        else if(name === '신청중회차'){ 수정신청중회차변경(value)}
    }

    const onSubmit = (event) => {
        event.preventDefault();
        db.collection('매칭결과변수').doc('variableInfo').update({
            마감시간: 수정마감시간,
            신청중: 수정신청중회차,
            ...
            <div className='매칭현황'>
                ...
                <div>
                    ...
                    <button onClick={매칭현황조회}>{회차}회차 조회</button>
                    {
                        ...
                        <div>
                            <h2>클로버</h2>
                            총 매칭 수: {클로버현황[0]+클로버현황[1]+클로버현황[2]} <br/>
                            zero: {클로버현황[0]} <br/>
                            half: {클로버현황[1]} <br/>
                            ssuccess: {클로버현황[2]} <br/>
                            매칭성공률: {클로버현황[2]/(클로버현황[0]+클로버현황[1]+클로버현황[2])*100}% <br/><br/>
                            
                            <h2>라일락</h2>
                            총 매칭 수: {라일락현황[0]+라일락현황[1]+라일락현황[2]} <br/>
                            zero: {라일락현황[0]} <br/>
                            half: {라일락현황[1]} <br/>
                            ssuccess: {라일락현황[2]} <br/>
                            매칭성공률: {라일락현황[2]/(라일락현황[0]+라일락현황[1]+라일락현황[2])*100}% <br/><br/>

                            <h2>데이지</h2>
                            총 매칭 수: {데이지현황[0]+데이지현황[1]+데이지현황[2]} <br/>
                            zero: {데이지현황[0]} <br/>
                            half: {데이지현황[1]} <br/>
                            ssuccess: {데이지현황[2]} <br/>
                            매칭성공률: {데이지현황[2]/(데이지현황[0]+데이지현황[1]+데이지현황[2])*100}% <br/>
                        </div>
                        : 
